...
function curlCsv(url: string): string {
  for (let attempt = 1; ; attempt++) {
    try {
      return execFileSync('curl', ['-sS', '--fail', '-g', '-A', 'Mozilla/5.0 (civic-superagents)', '-H', 'Accept: application/vnd.sdmx.data+csv;version=2.0.0', url], { maxBuffer: 64 * 1024 * 1024 }).toString('utf8')
    } catch (e) {
...
async function main() {
  ...
  const fonteId = await recordFonte({
    ...
    url: urlFor('P_100CH_Y0_2'), snapshot_date: SNAP, formato: 'sdmx-csv', license: 'CC-BY (ISTAT)',
    latest_usable_year: ANNO, granularita: 'comune', note_path: 'notes/istat-welfare.md',
    quirks: "SDMX v2 (curl -g); REF_AREA=codice_istat; indicatore=DATA_TYPE; OR-list '+' rotto -> una query per indicatore; P_100CH puo superare 100; CL_ITTER107 contiene comuni soppressi",
  })
