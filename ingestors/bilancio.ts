...
const YEARS = process.env.BDAP_YEARS ? process.env.BDAP_YEARS.split(',').map(Number) : NATIONAL || process.env.BDAP_REGIONI ? [2024] : [2022, 2023, 2024]
...
function discoverZipUrl(year: number, country: string): string | null {
  ...
  const out = execFileSync('curl', ['-sS', '--fail', '-g', '-A', UA, '-e', REFERER, api], { maxBuffer: 4 * 1024 * 1024 }).toString('utf8')
...
async function loadOne(country: string, year: number, valid: Set<string>, byName: Map<string, string>): Promise<number> {
  ...
  execFileSync('curl', ['-sS', '--fail', '-A', UA, '-e', REFERER, '-o', zipPath, url], { maxBuffer: 1024 * 1024 })
  ...
  const storage_path = await landSnapshot(SRC, `${year}/${slug}`, `Riepilogo_Missioni.csv`, Buffer.from(text, 'utf8'), 'text/csv')
  const fonteId = await recordFonte({
    ...
    url, snapshot_date: SNAP, storage_path, formato: 'csv-zip', license: 'CC-BY-3.0',
