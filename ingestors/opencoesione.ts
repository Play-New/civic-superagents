...
const URL = process.env.COESIONE_URL ||
  (NATIONAL
...
const DATASET = NATIONAL ? 'progetti_esteso_2021-2027' : 'progetti_esteso_LOM_2021-2027'
...
async function main() {
  ...
  execFileSync('curl', ['-sSL', '--fail', '-A', 'Mozilla/5.0 (civic-superagents)', '-o', zipPath, URL], { maxBuffer: 1024 * 1024 })
  ...
  const storage_path = await landSnapshot(SRC, SNAP, `${DATASET}.zip`, readFileSync(zipPath), 'application/zip')
  const fonteId = await recordFonte({
    source: SRC, dataset_id: DATASET,
    titolo: `OpenCoesione — progetti coesione ${NATIONAL ? 'Italia' : 'Lombardia'} (ciclo 2021-2027)`,
  ...
  for await (const rec of parser as AsyncIterable<string[]>) {
    ...
    projects.push({
      ...
      ciclo: rec[idx.OC_DESCR_CICLO]?.trim() || null, importo_finanziato: num(rec[idx.FINANZ_TOTALE_PUBBLICO]),
      importo_pagato: num(rec[idx.TOT_PAGAMENTI]), stato: rec[idx.OC_STATO_PROGETTO]?.trim() || null,
  ...
  console.log(`opencoesione (${NATIONAL ? 'NAZIONALE' : 'Lombardia'}) done. progetti:`, np, '| link:', nl, '| multi-comune:', multi)
  await sql.end()
}
